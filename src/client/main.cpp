...
// 主聊天页面程序
void mainMenu(int);  // 登录成功的主要聊天界面

// 聊天客户端程序实现，main线程用作发送线程，子线程用作接收线程，（想发就发，不影响我发不发，子线程用来接受）
int main(int argc, char **argv) {
  ...
  for (;;) {
    ...
    string input;
    getline(cin, input);
    int choice = atoi(input.c_str());  // 转换成数字
    ...
    switch (choice) {
      ...
      case 2:  // register业务
      {
        ...
        char pwd[50] = {0};
        cout << "username:";
        cin.getline(name, 50);  // cin>>对zhang san，只读取zhangsan， 空格停止了
        cout << "userpassword:";
        cin.getline(pwd, 50);
        ...
        // else {
        //   char buffer[1024] = {0};
        //   len = revc(clientfd, buffer, 1024, 0);
        //   if (-1 == len) {
...
void readTaskHandler(int clientfd) {
  for (;;) {
    ...
    while (true) {
      ...
      fullData.append(buffer, len);  // 拼接数据

      // 检查 JSON 是否完整
      if (fullData.front() == '{' && fullData.back() == '}') {
        break;  // 说明 JSON 解析不出错，可以退出循环
      }
    }

    // 接收ChatServer转发的数据，反序列化生成json数据对象
    json js = json::parse(fullData);
...
void showCurrentUserData() {
  ...
  if (!g_currentUserGroupList.empty()) {
    for (Group &group : g_currentUserGroupList) {
      cout << "GroupName: " << group.getId() << " " << group.getName() << " "
           << group.getDesc() << endl;
...
void mainMenu(int clientfd) {
  ...
  while (isMainMenuRunning) {
    ...
    if (-1 == idx) {  // 如果没有 `:`, 说明是简单命令（如 `help`）
      command = commandbuf;
    } else {
      command = commandbuf.substr(0, idx);  // 提取 `:` 之前的部分作为命令
    }
    auto it = commandHandlerMap.find(
        command);  // 在 `commandHandlerMap` 中查找是否存在该命令
