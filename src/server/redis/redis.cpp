...
bool Redis::connect() {
  ...
  // 在单独的线程中，监听通道上的事件，有消息给业务层进行上报， subscribe阻塞
...
bool Redis::publish(int channel, string message) {
  redisReply *reply = (redisReply *)redisCommand(  // 敲命令
...
// 为何subscribe和unsubscribe复杂

// 一个chatserver四个线程， 一个IO线程+三个work线程

// 向redis指定的通道subscribe订阅消息
bool Redis::subscribe(int channel) {  // 写到本地缓存->redis-server
